  ...
  (define (generate-primops)
    ...
    (for ([op list_of_cs])
      (let* ([label1 (fresh-offset op)]
        ...
        (if (eq? op "procedure-arity")
            (set! list_operations (foldr cons list_operations (list `( ,label1 (lambda (,var1) (if (procedure? ,var1) (,(string->symbol (string-append "unsafe-" op)) ,var1)
                                                                                                         (error ,error-code1)
                                                                                                         ))))))
            (set! list_operations (foldr cons list_operations (list `( ,label1 (lambda (,var1) (if (pair? ,var1) (,(string->symbol (string-append "unsafe-" op)) ,var1)
                                                                                                         (error ,error-code1)
                                                                                                         )))))))))
    ...
    (for ([op list_of_huhs])
      (let* ([label1 (fresh-offset op)]
        ...
        (dict-set! OPERATIONS op label1) ;update dictionary
        (set! list_operations (foldr cons list_operations (list `( ,label1 (lambda (,var1) (,(string->symbol op) ,var1))))))))
    ...
    (for ([b list_of_both])
      (let* ([label1 (fresh-offset b)]
        ...
        (set! list_operations (foldr cons list_operations (list `( ,label1 (lambda (,var1 ,var2) (,(string->symbol b) ,var1 ,var2))))))))
...
(define (letrec-seq b)
  ...
  (define (substitute-seq b)
    (match b
      ...
      [	`(letrec (,alc ...) ,e2)`(letrec (,@(map letrec-seq alc)) ,(substitute-seq e2))]
      [else b]
